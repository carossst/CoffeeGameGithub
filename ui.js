...
void function () {
  ...
  function formatExplanationForDisplay(raw, cfg, termEn) {
    ...
    if (en) {
      ...
      out = out.replaceAll(enEsc, `<strong>${enEsc}</strong>`);
    }
  ...
  UI.prototype.answer = function (choiceBool) {
    ...
    try {
      ...
      if (liveEl) {
        ...
        const parts = [verdictText, termEn, explanation].filter(Boolean);
        ...
        window.setTimeout(() => {
          ...
          try {
            ...
            if (nextItem) {
              ...
              if (words.length >= 1) {
                words[0].textContent = String(nextItem.termEn || "").trim();
              }
    ...
    try {
      if (
        ...
        window.WT_Analytics.trackFunnel(
          ...
          window.WT_Analytics.inferUiContext(this, {
            mode,
            run_type: this._runtime.lastRun.runType || this._runtime?.runType || ""
          })
        );
      }
    } catch (_) { /* silent */ }
  ...
  UI.prototype.render = function () {
    ...
    if (this.state !== STATES.PLAYING) {
      try {
        ...
        this.appEl.classList.remove("wt-fade--out");
        this.appEl.classList.remove("transitioning");
      } catch (_) { }
    }
    ...
    try {
      ...
      // exceeded" and landingViewed inflated ~40x per page load).
...
`;
  ...
  function buildEndModeCopy(ctx) {
    const {
      ...
      lastRun, vars, storage, runtime, w, poolSize
    } = ctx;
    ...
    if (isBonus) {
      ...
      let activeMistakes = clampInt(vars?.backlog, 0, 99999);
      if (!activeMistakes && storage && typeof storage.getActiveMistakesCount === "function") {
        try { activeMistakes = clampInt(storage.getActiveMistakesCount(), 0, 99999); } catch (_) { activeMistakes = 0; }
      }
  ...
  function buildEndMistakesRecap(ctx) {
    const { isRun, isPractice, isBonus, lastRun, maxChances, bonusW, practiceW, end, runtime, ui, cfg } = ctx;
...
`;
  ...
  UI.prototype._renderEnd = function () {
    ...
    const modeCopy = buildEndModeCopy({
      ...
      cfg, bonusW, practiceW, end,
      scoreFP, totalPresented, seen,
      lastRun, vars, storage: this.storage, runtime: this._runtime, w, poolSize
    });
    const {
      endLineTpl,
      bonusLevel,
    ...
    const mistakesRecapHtml = buildEndMistakesRecap({
      isRun, isPractice, isBonus, lastRun, maxChances, bonusW, practiceW, end,
...
`;
    ...
    const microLinesHtml = buildEndMicroLines({
      isRun, premium, end, runtime: this._runtime, lastRun, pbLine,
...
`;
    ...
    if (this._runtime.feedbackPending && this._runtime.lastAnswer) {
      ...
      const explanationHtml = stableExplanation
        ? `<p class="wt-explanation">${formatExplanationForDisplay(stableExplanation, cfg, termEn)}</p>`
        : "";
